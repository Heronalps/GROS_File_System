...
int gros_readdir(const char* path, void* buf, fuse_fill_dir_t filler, off_t offset, struct fuse_file_info* fi);
...
int gros_read(const char* path, char *buf, size_t size, off_t offset, struct fuse_file_info* fi);

// As for read above, except that it can't return 0.
int gros_write(const char* path, char *buf, size_t size, off_t offset, struct fuse_file_info* fi);
...
int gros_fsync(const char* path, int isdatasync, struct fuse_file_info* fi);

// Like fsync, but for directories.
int gros_fsyncdir(const char* path, int isdatasync, struct fuse_file_info* fi);
...
int gros_lock(const char* path, struct fuse_file_info* fi, int cmd, struct flock* locks);
...
int gros_poll(const char* path, struct fuse_file_info* fi, struct fuse_pollhandle* ph, unsigned* reventsp);

static struct fuse_operations gros_oper = {
    .init        = gros_init,
    .destroy     = gros_destroy,
    .getattr     = gros_getattr,
    .fgetattr    = gros_fgetattr,
    .access      = gros_access,
    .readlink    = gros_readlink,
    .readdir     = gros_readdir,
    .mknod       = gros_mknod,
    .mkdir       = gros_mkdir,
    .symlink     = gros_symlink,
    .unlink      = gros_unlink,
    .rmdir       = gros_rmdir,
    .rename      = gros_rename,
    .link        = gros_link,
    .chmod       = gros_chmod,
    .chown       = gros_chown,
    .truncate    = gros_truncate,
    .ftruncate   = gros_ftruncate,
    .utimens     = gros_utimens,
    .create      = gros_create,
    .open        = gros_open,
    .read        = gros_read,
    .write       = gros_write,
    .statfs      = gros_statfs,
    .release     = gros_release,
    .opendir     = gros_opendir,
    .releasedir  = gros_releasedir,
    .fsync       = gros_fsync,
    .flush       = gros_flush,
    .fsyncdir    = gros_fsyncdir,
    .lock        = gros_lock,
    .bmap        = gros_bmap,
    .ioctl       = gros_ioctl,
    .poll        = gros_poll,
#ifdef HAVE_SETXATTR
    .setxattr    = gros_setxattr,
    .getxattr    = gros_getxattr,
    .listxattr   = gros_listxattr,
    .removexattr = gros_removexattr,
#endif
